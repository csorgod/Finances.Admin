...
if(this.validateNumber){if(isNaN(value))return!1;arguments[0]=parseFloat(arguments[0]);return this.validateNumber.apply(this,arguments)}
...
if(!tester.test(value))return!1;if('number'===type){if(!/^any$/i.test(step||'')){var nb=Number(value);var decimals=Math.max(decimalPlaces(step),decimalPlaces(base));if(decimalPlaces(nb)>decimals)
...
this.constraints=constraints;this.constraintsByName=constraintsByName;for(var name in this.options)this.addConstraint(name,this.options[name],undefined,!0);return this._bindHtml5Constraints()},_bindHtml5Constraints:function _bindHtml5Constraints(){if(null!==this.element.getAttribute('required'))this.addConstraint('required',!0,undefined,!0);if(null!==this.element.getAttribute('pattern'))this.addConstraint('pattern',this.element.getAttribute('pattern'),undefined,!0);var min=this.element.getAttribute('min');var max=this.element.getAttribute('max');if(null!==min&&null!==max)this.addConstraint('range',[min,max],undefined,!0);else if(null!==min)this.addConstraint('min',min,undefined,!0);else if(null!==max)this.addConstraint('max',max,undefined,!0);if(null!==this.element.getAttribute('minlength')&&null!==this.element.getAttribute('maxlength'))this.addConstraint('length',[this.element.getAttribute('minlength'),this.element.getAttribute('maxlength')],undefined,!0);else if(null!==this.element.getAttribute('minlength'))this.addConstraint('minlength',this.element.getAttribute('minlength'),undefined,!0);else if(null!==this.element.getAttribute('maxlength'))this.addConstraint('maxlength',this.element.getAttribute('maxlength'),undefined,!0);var type=this.element.type;if('number'===type){return this.addConstraint('type',['number',{step:this.element.getAttribute('step')||'1',base:min||this.element.getAttribute('value')}],undefined,!0)}else if(/^(email|url|range|date)$/i.test(type)){return this.addConstraint('type',type,undefined,!0)}
...
result=$.Deferred().reject();return $.when(result)};return xhr.then(handleXhr,handleXhr)},priority:-1});Parsley.on('form:submit',function(){Parsley._remoteCache={}});Base.prototype.addAsyncValidator=function(){Utils.warnOnce('Accessing the method `addAsyncValidator` through an instance is deprecated. Simply call `Parsley.addAsyncValidator(...)`');return Parsley.addAsyncValidator.apply(Parsley,arguments)};Parsley.addMessages('en',{defaultMessage:"Esse valor parece inválido.",type:{email:"Esse campo não parece ser um e-mail válido.",url:"Esse campo não parece ser uma url válida.",number:"Esse campo não parece ser um número válido.",integer:"Esse campo não parece ser um número inteiro válido.",digits:"Esse campo precisa conter apenas dígitos.",alphanum:"Esse campo precisa ter valores alfa-numéricos."},notblank:"Esse valor não pode estar em branco.",required:"Esse valor é obrigatório.",pattern:"Esse valor parece ser inválido.",min:"Esse valor precisa ser maior ou igual a %s.",max:"Esse valor precisa ser menor ou igual a %s.",range:"Esse valor precisa estar entre %s e %s.",minlength:"Esse valor é muito curto. Ele precisa ter %s caracteres ou mais.",maxlength:"Esse valor é muito longo. Ele precisa ter %s caracteres ou menos.",length:"Esse valor é inválido. Ele precisa ter entre %s e %s caracteres.",mincheck:"Você precisa selecionar pelo menos %s opções.",maxcheck:"Você precisa selecionar %s opções ou menos.",check:"Você precisa selecionar entre %s e %s opções.",equalto:"Os valores precisam ser iguais."});Parsley.setLocale('en');function InputEvent(){var _this14=this;var globals=window||global;_extends(this,{isNativeEvent:function isNativeEvent(evt){return evt.originalEvent&&evt.originalEvent.isTrusted!==!1},fakeInputEvent:function fakeInputEvent(evt){if(_this14.isNativeEvent(evt)){$(evt.target).trigger('input')}},misbehaves:function misbehaves(evt){if(_this14.isNativeEvent(evt)){_this14.behavesOk(evt);$(document).on('change.inputevent',evt.data.selector,_this14.fakeInputEvent);_this14.fakeInputEvent(evt)}},behavesOk:function behavesOk(evt){if(_this14.isNativeEvent(evt)){$(document).off('input.inputevent',evt.data.selector,_this14.behavesOk).off('change.inputevent',evt.data.selector,_this14.misbehaves)}},install:function install(){if(globals.inputEventPatched){return}
